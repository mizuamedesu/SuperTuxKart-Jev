...
namespace
{
class JevStartDialog : public GUIEngine::ModalDialog
{
private:
    bool m_started;
...
public:
    JevStartDialog()
        : GUIEngine::ModalDialog(0.62f, 0.58f,
              GUIEngine::MODAL_DIALOG_LOCATION_CENTER),
          m_started(false)
    {
        World* world = World::getWorld();
        if (world && world->getPhase() != World::IN_GAME_MENU_PHASE)
            world->schedulePause(World::IN_GAME_MENU_PHASE);

        loadFromFile("jev_start_dialog.stkgui");
        getWidget<GUIEngine::ButtonWidget>("start")
    ...
    {
        // Keep the race and API gate closed until the explicit start action.
        return false;
    }

    virtual GUIEngine::EventPropagation processEvent(
...
{
    m_enabled = true;
    m_jev_start_dialog_shown = false;
...
void RaceGUI::reset()
{
    ...
    m_jev_start_dialog_shown = false;
    if (JevController::isWaitingForStart() && World::getWorld())
...
{
#ifndef SERVER_ONLY
    if (JevController::isWaitingForStart() &&
        !m_jev_start_dialog_shown &&
        !GUIEngine::ModalDialog::isADialogActive())
    {
        m_jev_start_dialog_shown = true;
        new JevStartDialog();
    }
...
//-----------------------------------------------------------------------------
/** Draws live Jev state/cost and a separate fighting-game-style control
 *  history over the real race view. */
void RaceGUI::drawJevDashboard()
{
#ifndef SERVER_ONLY
    World* world = World::getWorld();
    if (!world)
    ...
    const JevController* jev = NULL;
    const World::KartList& karts = world->getKarts();
    for (World::KartList::const_iterator i = karts.begin(); i != karts.end(); ++i)
    {
        if (!i->get())
            continue;
        jev = dynamic_cast<const JevController*>(i->get()->getController());
        if (jev)
            break;
    }
    if (!jev || !jev->hasDashboard())
        return;

    gui::ScalableFont* font = GUIEngine::getSmallFont();
    if (!font)
        return;
    const float previous_scale = font->getScale();
    font->setBlackBorder(true);

    const core::dimension2du screen = irr_driver->getActualScreenSize();
    ...
    font->setScale(0.64f);
    const std::vector<std::string>& lines = jev->getDashboardLines();
    const int line_height = font->getHeightPerLine() + 4;
    int text_width = 0;
    for (unsigned int i = 0; i < lines.size(); i++)
    {
        const core::stringw line(lines[i].c_str());
        text_width = std::max(text_width,
            static_cast<int>(font->getDimension(line.c_str()).Width));
    }
    const int panel_width = std::min(screen_width - edge * 2,
                                     text_width + padding * 2);
    const int panel_height = line_height * static_cast<int>(lines.size())
                           + padding * 2;
    const int left = edge;
    const int top = edge;
    const core::rect<s32> panel(left, top, left + panel_width,
                                top + panel_height);
    GL32_draw2DRectangle(video::SColor(205, 8, 13, 22), panel, NULL);
    GL32_draw2DRectangle(video::SColor(255, 34, 211, 238),
        core::rect<s32>(left, top, left + 4, top + panel_height), NULL);

    FontDrawer::startBatching();
    for (unsigned int i = 0; i < lines.size(); i++)
    {
        video::SColor color(255, 230, 236, 242);
        if (i == 0)
            color = video::SColor(255, 70, 225, 245);
        else if (i == 2 || i == 4 || i == 5)
            color = video::SColor(255, 255, 216, 96);
        else if (i == 7)
            color = video::SColor(255, 109, 235, 161);
        else if (lines[i].find("OFF TRACK") != std::string::npos)
            color = video::SColor(255, 255, 105, 105);

        const int y = top + padding + static_cast<int>(i) * line_height;
        font->draw(core::stringw(lines[i].c_str()),
            core::rect<s32>(left + padding, y, left + panel_width - padding,
                            y + line_height),
            color, false, false, NULL, true);
    }
    FontDrawer::endBatching();

    if (jev->hasInputHistory())
    {
        font->setScale(0.60f);
        const std::vector<std::string>& history =
            jev->getInputHistoryLines();
        const core::stringw title("CONTROL INPUT HISTORY  |  NEWEST FIRST");
        const int history_line_height = font->getHeightPerLine() + 5;
        int history_text_width = static_cast<int>(
            font->getDimension(title.c_str()).Width);
        for (unsigned int i = 0; i < history.size(); i++)
        {
            const core::stringw line(history[i].c_str());
            history_text_width = std::max(history_text_width,
                static_cast<int>(font->getDimension(line.c_str()).Width));
        }

        const int history_width = std::min(screen_width - edge * 2,
            history_text_width + padding * 2);
        const int history_height = history_line_height *
            (static_cast<int>(history.size()) + 1) + padding * 2;
        int history_left = screen_width - edge - history_width;
        int history_top = edge;
        if (history_left < left + panel_width + 12)
            history_top = top + panel_height + 12;
        if (history_top + history_height > screen_height - edge)
            history_top = std::max(edge, screen_height - edge - history_height);

        const core::rect<s32> history_panel(history_left, history_top,
            history_left + history_width, history_top + history_height);
        GL32_draw2DRectangle(video::SColor(215, 8, 13, 22),
                             history_panel, NULL);
        GL32_draw2DRectangle(video::SColor(255, 255, 161, 62),
            core::rect<s32>(history_left, history_top,
                            history_left + 4,
                            history_top + history_height), NULL);

        FontDrawer::startBatching();
        font->draw(title,
            core::rect<s32>(history_left + padding, history_top + padding,
                history_left + history_width - padding,
                history_top + padding + history_line_height),
            video::SColor(255, 255, 184, 91), false, false, NULL, true);

        for (unsigned int i = 0; i < history.size(); i++)
        {
            const int y = history_top + padding + history_line_height *
                (static_cast<int>(i) + 1);
            GL32_draw2DRectangle(
                i % 2 == 0 ? video::SColor(120, 28, 39, 53)
                           : video::SColor(90, 18, 28, 40),
                core::rect<s32>(history_left + 8, y,
                    history_left + history_width - 8,
                    y + history_line_height - 1), NULL);

            const std::string& value = history[i];
            const core::stringw row(value.empty() ? "--" : value.c_str());
            video::SColor color = value.empty()
                ? video::SColor(110, 160, 170, 181)
                : video::SColor(255, 232, 238, 245);
            if (value.find("R+") != std::string::npos)
                color = video::SColor(255, 255, 105, 105);
            else if (value.find("G!") != std::string::npos)
                color = video::SColor(255, 255, 139, 226);
            else if (value.find("[<") != std::string::npos)
                color = video::SColor(255, 92, 214, 255);
            else if (value.find("[>") != std::string::npos)
                color = video::SColor(255, 255, 191, 92);

            font->draw(row,
                core::rect<s32>(history_left + padding, y,
                    history_left + history_width - padding,
