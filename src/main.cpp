...
void cmdLineHelp()
{
    ...
    "       --jev-controller   Drive local player 1 from the local Jev JSON bridge.\n"
    "       --jev-state-port=N UDP telemetry destination (default 19736).\n"
    "       --jev-control-port=N UDP control listen port (default 19737).\n"
    "       --jev-telemetry-hz=N Telemetry datagrams per second (default 10).\n"
    "       --jev-timeout-ms=N Stop safely after this long without commands (default 2500).\n"
...
int handleCmdLine(bool has_server_config, bool has_parent_process)
{
    ...
    const bool use_jev_controller = CommandLine::has("--jev-controller");
    int jev_state_port = 19736;
    int jev_control_port = 19737;
    int jev_telemetry_hz = 10;
    int jev_timeout_ms = 2500;
    CommandLine::has("--jev-state-port", &jev_state_port);
    CommandLine::has("--jev-control-port", &jev_control_port);
    CommandLine::has("--jev-telemetry-hz", &jev_telemetry_hz);
    CommandLine::has("--jev-timeout-ms", &jev_timeout_ms);
    if (use_jev_controller &&
        !JevController::configure(jev_state_port, jev_control_port,
                                  jev_telemetry_hz, jev_timeout_ms))
    {
        return 0;
    }
